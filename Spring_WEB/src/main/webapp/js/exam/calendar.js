...
//            info.el.style.color = categoryColors[category];
            // info.el.style.backgroundColor = categoryColors[category];
            
/*             var categoryColors = {
                '공지사항': { color : 'blue', backgroundColor: 'blue', textColor: 'white' },
                '개인일정': { color : 'green', backgroundColor: 'green', textColor: 'white' },
                '연차/휴가': { color : 'red', backgroundColor: 'red', textColor: 'white' },
                '외근/출장': { color : 'orange', backgroundColor: 'orange', textColor: 'white' }
            };
            var colors = categoryColors[info.event.extendedProps.category] || { backgroundColor: 'gray', textColor: 'black' };

            var start = new Date(info.event.start);
            var end = new Date(info.event.end);
            var isMultiDayEvent = (end - start) > (24 * 60 * 60 * 1000);

            if (isMultiDayEvent) {
                info.el.style.backgroundColor = colors.backgroundColor;
                info.el.style.borderColor = colors.backgroundColor;
                info.el.style.textColor = colors.textColor;
            }
